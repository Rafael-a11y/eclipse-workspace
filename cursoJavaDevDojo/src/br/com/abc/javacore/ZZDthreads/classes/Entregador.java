...
public class Entregador implements Runnable {
	...
	public void run() {
		...
		System.out.println(Thread.currentThread().getName() + " começando o trabalho de entrega");
		...
		while(aberta || qtdPendente > 0) {
			try {
				...
				if(email != null) {
					System.out.println("Nome da classe " + getClass().getName() + nomeThread + " enviando email para " + email);
					Thread.sleep(2000);
